...
import { useAuth } from "../../contexts/AuthContext"; // To access logout and user role

const DashboardLayout = ({ title, description, children }) => {
  const { user, logout } = useAuth(); // Destructure user for role check
          ...
          <nav>
            <ul className="flex items-center space-x-6 md:space-x-8 text-lg font-semibold">
              {user?.role === "admin" && (
                <li>
                  <Link href="/admin">
                    <div className="text-purple-400 hover:text-white transition duration-300 transform hover:scale-105 cursor-pointer">
                      Admin Panel
                    </div>
                  </Link>
                </li>
