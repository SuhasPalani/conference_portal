...
from flask_jwt_extended import create_access_token # <--- ADDED THIS IMPORT
...
def connect_with_mentor(user_id):
    ...
    if str(requester_id) == user_id:
        return jsonify({"success": False, "message": "You cannot connect with yourself."}), 400

    try:
        mentor = User.find_by_id(user_id)
        if not mentor or mentor.role != 'Mentor' or mentor.status != 'active':
            return jsonify({"success": False, "message": "Mentor not found or not active."}), 404

        # In a real app, you'd record this connection request in the DB
        # For now, simulate success
        current_app.logger.info(f"User {requester_id} attempting to connect with mentor {user_id}")
        return jsonify({"success": True, "message": "Connection request sent to mentor."}), 200
    except Exception as e:
...
# --- Team Routes ---
@community_bp.route('/teams', methods=['GET', 'OPTIONS'])
@jwt_required() # Any logged-in user can search for teams
def get_teams():
    if request.method == 'OPTIONS':
        return '', 200
    ...
    try:
        ...
        for team in teams:
            ...
            # Optionally fetch leader's name for display
            leader_user = User.find_by_id(team.leader_id)
            team_dict['leader_name'] = leader_user.full_name if leader_user else "Unknown"
            teams_data.append(team_dict)
        
        return jsonify({"success": True, "teams": teams_data}), 200
    except Exception as e:
...
def create_team():
    ...
    if not all([name, description, category, max_members is not None]):
        return jsonify({"success": False, "message": "Missing required team fields."}), 400
    
    try:
        # Check if user is already in a team
        user_in_db = User.find_by_id(leader_id)
        if user_in_db.team_id:
            return jsonify({"success": False, "message": "You are already part of a team. Please leave your current team before creating a new one."}), 409

        # Check if team name already exists
        existing_team = mongo.db.teams.find_one({"name": name})
        if existing_team:
            return jsonify({"success": False, "message": "A team with this name already exists."}), 409

        new_team = Team(name, description, category, leader_id, max_members, skills_needed)
        ...
        return jsonify({"success": False, "message": "An error occurred while creating the team."}), 500
...
def join_team(team_id):
    ...
    try:
        if not ObjectId.is_valid(team_id):
            return jsonify({"success": False, "message": "Invalid team ID format."}), 400
        ...
        if team.leader_id == ObjectId(requester_id):
            return jsonify({"success": False, "message": "You are the leader of this team."}), 400

        if ObjectId(requester_id) in team.members:
            return jsonify({"success": False, "message": "You are already a member of this team."}), 409
        ...
        if user_in_db.team_id:
            return jsonify({"success": False, "message": "You are already part of another team. Please leave it before joining a new one."}), 409
        ...
        return jsonify({"success": False, "message": "An error occurred while joining the team."}), 500
...
def leave_team(team_id):
    ...
    try:
        ...
        if ObjectId(user_id) not in team.members:
            return jsonify({"success": False, "message": "You are not a member of this team."}), 400
        
        if team.leader_id == ObjectId(user_id):
            # Option: Disband team if leader leaves and no members, or transfer leadership
            return jsonify({"success": False, "message": "Leaders cannot directly 'leave' a team; they must disband it or transfer leadership."}), 403
        ...
        return jsonify({"success": False, "message": "An error occurred while leaving the team."}), 500
...
def disband_team(team_id):
    ...
    try:
        ...
        if team.leader_id != ObjectId(user_id):
            return jsonify({"success": False, "message": "Only the team leader can disband the team."}), 403
        ...
        return jsonify({
            ...
            "user": updated_leader_user.to_dict() if updated_leader_user else None, # For immediate context update
        ...
        return jsonify({"success": False, "message": "An error occurred while disbanding the team."}), 500