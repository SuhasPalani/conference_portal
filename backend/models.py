...
class User:
    ...
    def find_mentors(search_term="", skill=""):
        ...
        if skill:
            query["expertise"] = skill # Find documents where 'expertise' array contains 'skill'
    ...
    def find_potential_teammates(current_user_id, search_term="", interests=[]):
        ...
        if interests:
            query["interests"] = {"$in": interests} # Match if any of user's interests are in the search list
...
class Team:
    ...
    def __init__(self, name, description, category, leader_id, max_members=5, skills_needed=None):
        self.name = name
        ...
        self.status = "Looking for members" # "Looking for members", "Full", "Completed", etc.
        self._id = None
    ...
    def to_dict(self):
        ...
        return {
            ...
            "members": [str(member_id) for member_id in self.members], # Convert ObjectIds to strings
    ...
    def from_mongo_doc(doc):
        ...
        team = Team(
            ...
            leader_id=doc.get("leader_id"), # This will be an ObjectId from DB
            max_members=doc.get("max_members"),
    ...
    def get_all_teams(search_term="", category=""):
        """Fetches all teams from the database with optional search and category filtering."""
        query = {}
        if search_term:
            query["$or"] = [
        ...
        teams_cursor = mongo.db.teams.find(query)
...
def ensure_mongo_indexes():
    ...
    try:
        ...
        # Index for blacklist tokens (sparse to allow multiple null JTIs if that happens)
